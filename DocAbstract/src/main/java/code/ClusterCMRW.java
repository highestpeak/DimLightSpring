...
public class ClusterCMRW {
    ...
    public void calcScSim() {
        ...
        for (int i = 0; i < myDoc.snum; i++) {
            scSim[i] = myDoc.calcCos(myDoc.sVector.get(i), myDoc.sTf.get(i), myDoc.wordLen.get(i),
                    cluster.get(belong[i]), cTf.get(belong[i]), cLen.get(belong[i]));
    ...
    public void Kmeans() {
        ...
        while (change) {
            ...
            for (int i = 0; i < myDoc.snum; i++) {
                double minDis = 0;
                for (int j = 0; j < k; j++) {
                    double dis = myDoc.calcCos(myDoc.sVector.get(i), myDoc.sTf.get(i), myDoc.wordLen.get(i),
                            cluster.get(j), cTf.get(j), cLen.get(j));
                    if (dis > minDis) {
                        minDis = dis;
                        belong[i] = j;
                    }
